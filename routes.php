<?php
    ...
    {
        $locationArr = explode(",",$row["route_location"]);
        
        // Search the tables if we have any routes thats matches the form details
        if(!in_array($source, $locationArr) || !in_array($destination, $locationArr) || !(array_search($source, $locationArr) < array_search($destination, $locationArr)))
            continue;
        $count++;
    }
    ...
    <main id="container">
        ...
        <?php 
            ...
            while($row = mysqli_fetch_assoc($result))
            {
                ...
                $source_idx = array_search($source, $locationArr);
                $dest_idx = array_search($destination, $locationArr);

                $viaLocation = false;
                if($source_idx + 1 != $dest_idx)
                {
                    $viaLocation = implode(",", array_slice($locationArr, $source_idx + 1, $dest_idx - $source_idx));
                }
