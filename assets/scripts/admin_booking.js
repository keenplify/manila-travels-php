...
function showRouteSuggestions(evt)
{
  ...
  let suggestions = routeData.filter(({route_location}) =>   route_location.match(regex))
  .map(({route_id, route_location, route_dep_date, route_dep_time, route_step_cost}) => {
    const viaLocation = route_location.replace(regex, `<span class="hl">${this.value.toUpperCase()}</span>`);
    ...
    <span class="viaLocation-span">${viaLocation}</span>
...
function showSourceSuggestions(evt)
{
  ...
  const locationArr = convertToArray(routeSelected);
  /*
    1. last place cannot be a source, hence cannot be given in the suggestion
  */
  let suggestions = locationArr.filter(location => 
    location.match(regex) && locationArr.indexOf(location) !== locationArr.length - 1)
  .map(location => {
    const locationName = location.replace(regex, `<span class="hl">${this.value.toUpperCase()}</span>`);
...
function showDestinationSuggestions(evt)
{
  ...
  console.log(locationArr.indexOf(sourceSelected));
    for (let i = 0; i < locationArr.length; ++i)
    if(i > locationArr.indexOf(sourceSelected))
      suggestions.push(locationArr[i]);

  console.log(suggestions);
  suggestions = suggestions.filter(location => location.match(regex))
  .map(location => {
    const locationName = location.replace(regex, `<span class="hl">${this.value.toUpperCase()}</span>`);
    return `<li>${locationName}</li>`;
  }).join("");
...
function lockSuggestion(evt)
{
  ...
  else if(searchInput.id === "routeSearch")
  {
    ...
    // Converting comma separated cities to an array
    const locationArr = convertToArray(route_id);

    if(locationArr.length === 2)
    {
      document.querySelector("#sourceSearch").value = locationArr[0];
      document.querySelector("#destinationSearch").value = locationArr[1];
    }
  ...
  if(evt.target.nodeName == "TD" && !evt.target.className.includes("space") && !evt.target.className.includes("notAvailable"))
  {
    if(!selected_id || evt.target.dataset.name === selected_id)
    {
      ...
      if(selected_id)
      {
        ...
        const locationArr = convertToArray(document.querySelector("#routeSearch").dataset.id);

        console.log(sourceSelected, destSelected, locationArr);
        const stepCost = document.querySelector("#routeSearch").dataset.stepcost;

        const amount = (locationArr.indexOf(destSelected) - locationArr.indexOf(sourceSelected)) * parseInt(stepCost);
        console.log(locationArr.indexOf(destSelected), locationArr.indexOf(sourceSelected), stepCost);
