    ...
    <?php
        ...
        */
        if($loggedIn && $_SERVER["REQUEST_METHOD"] == "POST")
        {
            if(isset($_POST["submit"]))
            {
                ...
                */
                ...
                $busno = $_POST["busno"];
                $route_exists = exist_routes($conn,$Location,$depdate, $deptime);
                $route_added = false;
        
                if(!$route_exists)
                {
                    // Route is unique, proceed
                    $sql = "INSERT INTO `routes` (`route_location`,
                     `bus_no`, 
                     `route_dep_date`,
                     `route_dep_time`, `route_step_cost`, `route_created`) VALUES ('$Location','$busno', '$depdate','$deptime', '$cost', current_timestamp());";
            ...
            if(isset($_POST["edit"]))
            {
                ...
                $id_if_route_exists = exist_routes($conn,$Location,$depdate,$deptime);
        ...
        <?php
            ...
            <?php
                $busSql = "Select * from buses  where bus_assigned=0 AND status = 'Active'";
                    ...
                    <div class="modal-body">
                        <form id="addRouteForm" action="<?php echo $_SERVER['REQUEST_URI']; ?>" method="POST">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="Location" name="Location" placeholder="Comma Separated List" required>
                                <span id="error">
                            ...
                            <div class="mb-3">
                                ...
                                <div class="searchBus">
                                    <select class="form-select" name="busno" required>
                                        <option disabled selected>Select a Bus</option>
                                        <?php
                                            foreach ($arr as $bus) {
                                        ?>
                                            <option><?= $bus["bus_no"] ?></option>
                                        <?php
                                            }
                                        ?>
                                    </select>
